...
//When Android framework load too slow, the first boot sound audio path may not speaker. 
extern int Is_first_boot_flag ;					/* Jagan+ ... Jagan- */
...
{
	//Change to likely since it only false when first playing sound 
	if (likely(policy == SCHED_FIFO))
		return 1;
	return 0;
}
