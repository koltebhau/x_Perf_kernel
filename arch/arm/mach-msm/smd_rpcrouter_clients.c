...
*/
						    ...
						    smdrpcrouterclient_old_prio = current->rt_priority;
 						    ...
 						    {
						    	if ((sched_setscheduler(current, SCHED_FIFO, &smdrpcrouterclient_s)) < 0)
   						    		printk(KERN_ERR "[SEAN]uppp rpc_clients_thread fail\n");
   						    	else
						    ...
						    client->cb_xdr.in_buf, rc);
 						    ...
 						    if (!(unlikely(smdrpcrouterclient_old_policy == SCHED_FIFO) || unlikely(smdrpcrouterclient_old_policy == SCHED_RR)))
 						    {
  						    	...
  						    	if ((sched_setscheduler(current, smdrpcrouterclient_old_policy, &smdrpcrouterclient_v)) < 0)
   						    		printk(KERN_ERR "[SEAN]down rpc_clients_thread failed\n");
   						    	else
