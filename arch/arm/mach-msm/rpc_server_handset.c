...
#define HS_BAD_BATT_K     0x77
#define HS_CHARGING_ON_K  0x88
#define HS_CHARGING_OFF_K 0x89
#define HS_BAT_LOW_K      0xD1
...
static const uint32_t hs_key_map[] = {
    ...
    KEY(HS_BAD_BATT_K,    KEY_BATTERY),
    KEY(HS_CHARGING_ON_K, KEY_BATTERY),
    KEY(HS_CHARGING_OFF_K,KEY_BATTERY),
    KEY(HS_BAT_LOW_K,     KEY_BATTERY),
	0
};
...
static struct delayed_work      pwrkey_work;
...
static int __devinit hs_probe(struct platform_device *pdev)
{
  ...
  pwrkey_down = 0;
  INIT_DELAYED_WORK(&pwrkey_work, pwrkey_work_func);
  pwrkey_wqueue = create_singlethread_workqueue("pwrkey_workqueue");
  if(pwrkey_wqueue)
  {
    printk("[KEY]Enable feature: Power Key 8 sec, then power down!\n");
  }
  else
  {
    printk("[KEY]pwrkey_wqueue create FAIL!\n");
